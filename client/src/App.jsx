...
import MobileCardNav from './components/MobileCardNav';
...
export default function App() {
  ...
  const cliMode = useMemo(() => {
    if (typeof navigator === 'undefined') return false;
    return CLI_USER_AGENT_RE.test(navigator.userAgent || '');
  }, []);
  ...
  const toggleLanguage = useCallback(() => {
    setLanguage((prev) => {
      ...
      if (nextLang === 'en') writeLanguageCookie('en');
      else writeLanguageCookie('');
      return nextLang;
    });
  }, []);
  ...
  const loadPredictions = useCallback(async (stopId, { force = false } = {}) => {
    setError('');
    try {
  ...
  const loadAutomationStatus = useCallback(async () => {
    try {
      ...
      if (!res.ok) return;
      setAutomationStatus(await res.json());
    } catch (_err) {
      // no-op
    }
  }, []);

  const triggerAutomation = useCallback(async (action) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [cfgRes, stopsRes] = await Promise.all([fetch('/api/config'), fetch('/api/stops')]);
        if (cfgRes.ok && mounted) {
          const cfg = await cfgRes.json();
          setWalkTimeMinutes((prev) => toPositiveNumber(cfg?.walkTimeMinutes, prev));
          setRefreshIntervalMs((prev) => toPositiveNumber(cfg?.refreshIntervalMs, prev));
        }
        if (!stopsRes.ok) throw new Error(`Failed to load stops (${stopsRes.status})`);
        const stopsJson = await stopsRes.json();
        const list = Array.isArray(stopsJson?.stops) ? stopsJson.stops : [];
        if (mounted) {
          setStops(list);
  ...
  }, [selectedStopId, refreshIntervalMs, loadPredictions]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setCardTitleVisible(false), 700);
    return () => clearTimeout(timer);
  }, [activeMobileCard]);

  useEffect(() => {
    if (!isMobileViewport) setMobileCardIndex(0);
  }, [isMobileViewport]);
  ...
  useEffect(() => {
    ...
    const end = (event) => {
      ...
      if (Math.abs(delta) > MIN_SWIPE_DISTANCE) cycleMobileCard(delta < 0 ? 1 : -1);
      startX = null;
    };
  ...
  useEffect(() => {
    ...
    const handleTab = (event) => {
      ...
      if (promptShownRef.current || showAccessibleView) return;
  ...
  const annotatePredictions = useCallback((list) => list.map((prediction) => {
    const eventMs = getPredictionEventMs(prediction);
  ...
  const walkIndicator = useMemo(() => {
    ...
    if (leaveDeltaMs <= 0) {
      return {
        title: languageText.walk.leaveNow,
        subtitle: `${directionLabel} · ${headsign} · ${languageText.walk.trainIn} ${formatDuration(eventDeltaMs)}`,
      };
    }

    return {
      title: `${languageText.walk.leaveSoon} ${formatDuration(leaveDeltaMs)}`,
      subtitle: `${directionLabel} · ${headsign} · ${languageText.walk.trainIn} ${formatDuration(eventDeltaMs)}`,
    };
  }, [annotatedPrimary, languageText, walkBufferMs]);
  ...
  const heroBufferLabel = languageText.walk.walkBufferText(walkMinutesLabel, Math.round(refreshIntervalMs / 1000));
  const timetableHeadline = annotatedPrimary.length && annotatedPrimary[0].liveMinutes !== null
  ...
  const handleTrainPass = useCallback(async (direction, list) => {
    ...
    const predictedAtMs = Number.isFinite(prediction.eventMs) && prediction.eventMs > 0
      ? prediction.eventMs
      : minutes !== null
    ...
    try {
      const res = await fetch('/api/train-pass', {
        ...
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          stopId: selectedStopId || FALLBACK_STOP_ID,
          direction,
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json?.error || 'Unable to log train pass');
      const offsetMs = Number.isFinite(json?.offsetMs) ? json.offsetMs : null;
      const offsetLabel = offsetMs === null ? 'offset unknown' : `${offsetMs >= 0 ? '+' : ''}${Math.round(offsetMs / 1000)}s`;
  ...
  const nextAutomationLabel = (() => {
    ...
    const modeLabel = languageText.automation.modes[nextAutomationWindow.mode] || nextAutomationWindow.mode || 'Event';
    const when = formatRelative(Date.parse(nextAutomationWindow.startAt) - Date.now());
    return when === '—' ? '—' : `${modeLabel} ${languageText.automation.upcomingIn} ${when}`;
  })();

  useEffect(() => {
    if (typeof document === 'undefined') return;
    document.title = ['MBTA Tracker', heroTitle].filter(Boolean).join(' · ');
  }, [heroTitle]);
      ...
      {showAccessibleView ? (
        ...
        <div className="page min-h-screen bg-slate-50 text-slate-900" style={{ '--mbta-accent': dominantColor }}>
          <header className="header">
            <div className="brand"><h1>🔵 Suffolk Downs</h1></div>
            <div className="header-controls">
              ...
              <a className="version-chip" href={REPO_URL} target="_blank" rel="noreferrer">v{appVersion}</a>
            </div>
          </header>

          <main className="content">
            {error ? <div className="alert error">{languageText.alerts.error}: {error}</div> : null}
            {automationStatus?.lastActionError ? <div className="alert error">{languageText.alerts.automation}: {automationStatus.lastActionError}</div> : null}
            {!integrityOk ? (
              <div className="alert check-alert">
                {[!checklist.inbound && languageText.alerts.inboundIntegrity, !checklist.outbound && languageText.alerts.outboundIntegrity].filter(Boolean).join(' ')}
              </div>
            ) : null}
            ...
            <section className="flashcard-wrapper">
              <div className="flashcard-stack" ref={swipeRef}>
                <article className={`flashcard hero-card ${activeMobileCard === 'hero' ? 'flashcard--active' : ''}`}>
                  <div className="panel-heading" data-title-visible={activeMobileCard === 'hero' ? String(cardTitleVisible) : 'true'}>
                ...
                <article className={`flashcard timetable-card ${activeMobileCard === 'timetable' ? 'flashcard--active' : ''}`}>
                  <div className="panel-heading" data-title-visible={activeMobileCard === 'timetable' ? String(cardTitleVisible) : 'true'}>
                    <span className="panel-emoji" role="presentation">⬆️</span>
                    <div>
                  ...
                  <div className="timetable-section">
                    <div className="section-heading"><span className="section-title">{languageText.flashcards.inboundLabel}</span></div>
                    <PredictionList
                      items={annotatedPrimary}
                ...
                <article className={`flashcard wonderland-card ${activeMobileCard === 'wonderland' ? 'flashcard--active' : ''}`}>
                  <div className="panel-heading" data-title-visible={activeMobileCard === 'wonderland' ? String(cardTitleVisible) : 'true'}>
                    <span className="panel-emoji" role="presentation">↩️</span>
                    <div>
                ...
                <VolumePanel
                  active={activeMobileCard === 'volume'}
                  cardTitleVisible={cardTitleVisible}
                  languageText={languageText}
                  automationStateLabel={automationStateLabel}
                  automationStateClass={automationStateClass}
                  nextAutomationLabel={nextAutomationLabel}
                ...
                {isMobileViewport ? (
                  <MobileCardNav
                    ...
                    cycleMobileCard={cycleMobileCard}
