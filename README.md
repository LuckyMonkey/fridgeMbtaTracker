...
- **Mobile alarm clock view**: On phones the hero clock takes over the entire viewport (header/status now hidden) and shows the leave timer with friendly emojis; dots plus arrow buttons at the bottom cycle inbound/outbound/volume cards without any scrolling, and the entire experience stays on-screen like a single pot.
